...
map.on('click', function(e) {
 ...
 const url = `https://api.openweathermap.org/data/2.5/onecall?lat=${latitude}&lon=${longitude}&units=metric&appid=${apiKey}`;
  ...
  .then(data => {
    ...
    const futureThreeHours = futureHours.slice(0, 5); // get next 5 hours
    ...
    const futureThreeDays = data.daily.slice(1, 6); 

    const dailyForecast = futureThreeDays.map(day => {
      const date = new Date(day.dt * 1000).toLocaleDateString('en-US', { weekday: 'long' });
      return `<strong>${date}</strong>: ${day.weather[0].description}, Max: ${day.temp.max}°C, Min: ${day.temp.min}°C`;
    }).join('<br>');
    ...
    document.getElementById('popup').innerHTML = `
      ...
      <strong>Weather for the next 5 hours:</strong><br>${hourlyForecast}<br>
       <strong>Weather for the next 5 days:</strong><br>${dailyForecast}`;
  })
...
const scale = new mapboxgl.ScaleControl({
  maxWidth: 70, //size of the scale bar
  unit: "metric"
});
